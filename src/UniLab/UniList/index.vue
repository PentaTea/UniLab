      ...
      <el-main style="padding: 0">
            ...
            <template v-if="i == '$index'">
              ...
              >
                <template #default="scope">
                  <span>{{
                    scope.$index + (currentPageNow - 1) * pages.size + 1
                  }}</span>
                </template>
              </el-table-column>
            </template>
        ...
        >
          ...
          <template
            v-if="
              rawFormOption.text &&
                !rawFormOption.component &&
                !rawFormOption.form
            "
            #footer
          >
            <el-button @click="rawFormOption.reject(), (rawDialogShow = false)">
              取消
            </el-button>
            <el-button
              type="primary"
              @click="rawFormOption.resolve(), (rawDialogShow = false)"
            >
              确定
            </el-button>
          </template>
...
<script lang="ts">
  ...
  export default class UniList extends Options {
    ...
    get parserFormConf() {
      ...
      form.fields.forEach((item, i) => {
        ...
        if (conp) {
          ...
          else if (Array.isArray(conp))
            item.__config__.use = { [conp[0]]: conp[1] || true };
        }
      });
    ...
    match(type: "action" | "header", name) {
      return reduce(
        Object.values(
          pickBy(this[type + "Style"], (value, key) =>
